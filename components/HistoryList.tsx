...
const HistoryList = memo(function HistoryList({ history, onClear, onRestore }: HistoryListProps) {
  if (history.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="text-slate-500 dark:text-slate-400 font-medium text-sm">No recent history</p>
        <p className="text-slate-400 dark:text-slate-500 text-xs mt-1">Your calculations will appear here</p>
      </div>
    );
  }
          ...
          >
            <div className="flex flex-col">
              <div className="flex items-center gap-2 text-slate-800 dark:text-slate-200 font-semibold text-sm">
                <span>
                  {item.amountFrom}{' '}
                  <span className="text-xs text-slate-400 dark:text-slate-500 font-medium">{item.from}</span>
                </span>
                ...
                <span className="text-indigo-600 dark:text-indigo-400">
                  {item.amountTo}{' '}
                  <span className="text-xs text-indigo-400 dark:text-indigo-600 font-medium">{item.to}</span>
                </span>
              </div>
